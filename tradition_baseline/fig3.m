...
p6=plot(50,17,'p','MarkerEdgeColor',[1	0	0], 'MarkerFaceColor',[1	0	0],'MarkerSize',10,'LineWidth',2);
hold on
...
hold on
for i=1:1:length(x1)
   p1=plot(y1(i),x1(i),'o','MarkerSize',3,'MarkerEdgeColor',[0.15,0.15,0.15], 'MarkerFaceColor',[0.15,0.15,0.15]); 
end
for i=1:1:length(x2)
   plot(y2(i),x2(i),'o','MarkerSize',3,'MarkerEdgeColor',[1.00,0.41,0.16], 'MarkerFaceColor',[1.00,0.41,0.16]) ;
end
for i=1:1:length(x3)
   plot(y3(i),x3(i),'o','MarkerSize',4,'MarkerEdgeColor',[ 0.58,0.27,0.78], 'MarkerFaceColor',[ 0.58,0.27,0.78]) ;
end
for i=1:1:length(x4)
   p2=plot(y4(i),x4(i),'^','MarkerSize',3,'MarkerEdgeColor',[0.24,0.24,0.24], 'MarkerFaceColor',[0.24,0.24,0.24]); 
end
for i=1:1:length(x5)
   plot(y5(i),x5(i),'^','MarkerSize',4,'MarkerEdgeColor',[0.85,0.33,0.10], 'MarkerFaceColor',[0.85,0.33,0.10]) 
end
for i=1:1:length(x6)
   plot(y6(i),x6(i),'^','MarkerSize',4,'MarkerEdgeColor',[0.40,0.14,0.58], 'MarkerFaceColor',[ 0.40,0.14,0.58]) 
end
hold on
p4=plot(y1(1),x1(1),'+','MarkerEdgeColor',[0.50,0.50,0.50], 'MarkerFaceColor',[0,0,0],'MarkerSize',13, 'LineWidth',3);
hold on
p5=plot(y1(length(x1)),x1(length(x1)),'X','MarkerEdgeColor',[0.50,0.50,0.50], 'MarkerFaceColor',[0,0,0],'MarkerSize',13, 'LineWidth',3);
hold on
...
fileID = fopen('C:/Users/Administrator.DESKTOP-NLH290A/Desktop/2302_code/response_code/fig3/A算法/robot_path_1.txt', 'r');
% fileID="C:/Users/Administrator.DESKTOP-NLH290A/Desktop/2302_code/response_code/fig3/A算法/robot_path_1.txt";
data = textscan(fileID, '%f32,%f32', 'Delimiter', '，');
x1=data{1};
y1=data{2};
fclose(fileID);
fileID = fopen('C:/Users/Administrator.DESKTOP-NLH290A/Desktop/2302_code/response_code/fig3/A算法/robot_path_2.txt', 'r');
data = textscan(fileID, '%f,%f', 'Delimiter', '，');
x2=data{1};
y2=data{2};
fclose(fileID);
fileID = fopen('C:/Users/Administrator.DESKTOP-NLH290A/Desktop/2302_code/response_code/fig3/A算法/robot_path_3.txt', 'r');
data = textscan(fileID, '%f,%f', 'Delimiter', '，');
x3=data{1};
y3=data{2};
fclose(fileID);
for i=1:2:length(x1)
   p3=plot(y1(i),x1(i),'*','MarkerSize',5,'MarkerEdgeColor',[0.50,0.50,0.50], 'MarkerFaceColor',[0.50,0.50,0.50]); 
end
for i=1:2:length(x2)
   plot(y2(i),x2(i),'*','MarkerSize',6,'MarkerEdgeColor',[  0.93,0.69,0.13], 'MarkerFaceColor',[ 0.93,0.69,0.13 ]) ;
end
for i=1:2:length(x3)
   plot(y3(i),x3(i),'*','MarkerSize',5,'MarkerEdgeColor',[ 0.76,0.43,0.96], 'MarkerFaceColor',[0.76,0.43,0.96]) ;
end
...
axis equal
axis([0 100 0 100]);
legend([p1 p2 p3],{'Distance-Aware MA-DDPG ($\kappa_1=0$)','Communication-Aware MA-DDPG ($\kappa_1=0.1$)', '$A^{\ast}$ Search Algorithm'},'Location','NorthWest','Interpreter','latex','NumColumns',2)
...
legend(ah,[p4 p5 p6],{'MU Starting Point ','MU Destination','SU'},'Location','NorthEast','Interpreter','latex','NumColumns',4)
